...
def main():
  ...
  for target_idx in tqdm(range(num_targets), desc='Target ID', leave=False):
    ...
    for context_idx in tqdm(range(num_contexts), desc='Context ID',
      ...
      if args.wandb and context_idx>0:
        stacked_image = np.hstack((target_image, context_image, viz_results[0], viz_results[-1]))
        wandb.log({"diffused image": [wandb.Image(stacked_image, caption="original - context - stitched - diffused image")]})
